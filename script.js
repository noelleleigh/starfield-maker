...
const distFromLine = function distFromLine(P1x, P1y, P2x, P2y, x, y) {
  const doubleTriArea = (((P2y - P1y) * x) - ((P2x - P1x) * y)) + ((P2x * P1y) - (P2y * P1x));
  const absDoubleTriArea = Math.abs(doubleTriArea);
  const lineLength = Math.sqrt(((P2y - P1y) ** 2) + ((P2x - P1x) ** 2));
  return absDoubleTriArea / lineLength;
};
  ...
  Array.from(form.elements)
    ...
    .forEach((elem) => {
      ...
      if (elem.type === 'checkbox') {
        ...
        value = elem.value; // eslint-disable-line prefer-destructuring
      }
...
const drawGlowBand = function drawGlowBand(ctx, x1, y1, x2, y2, intensity, width, height) {
  ...
  const brightnessDist = Math.sqrt(((y2 - y1) ** 2) + ((x2 - x1) ** 2));
  const orthoX1 = Math.floor(x1 + ((brightnessDist / 2) * (brightnessVector[0] + orthoVector[0])));
  const orthoY1 = Math.floor(y1 + ((brightnessDist / 2) * (brightnessVector[1] + orthoVector[1])));
  const orthoX2 = Math.floor(x1 + ((brightnessDist / 2) * (brightnessVector[0] - orthoVector[0])));
  const orthoY2 = Math.floor(y1 + ((brightnessDist / 2) * (brightnessVector[1] - orthoVector[1])));
...
// Return a function that sets the outputElement textContent to the target value
const displayRangeOutput = outputElement => (event) => {
